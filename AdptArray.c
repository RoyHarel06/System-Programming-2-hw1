...
{
    PAdptArray adpt_arr = (PAdptArray) malloc(sizeof(struct AdptArray_));
...
Result SetAdptArrayAt(PAdptArray adpt_arr, int index, PElement element)
{
    ...
    {
        if (adpt_arr->size == 0)
            adpt_arr->elements = malloc((index+1)*sizeof(PElement));
        else
            adpt_arr->elements = realloc(adpt_arr->elements, (index+1)*sizeof(PElement));
...
PElement GetAdptArrayAt(PAdptArray adpt_arr, int index)
{
    ...
    if (adpt_arr->elements[index] == NULL)
        return NULL;

    return adpt_arr->copy_func(adpt_arr->elements[index]);
}
